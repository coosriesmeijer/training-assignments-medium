 ...
 * certain days. The rule mostly relies on tags on the volume to decide if the
...
public class OldDetachedVolumeRule extends AbstractRule implements Rule {
	...
	/**
	 * The date format used to print or parse the user specified termination
	 * date.
	 **/
	 ...
	 *            The number of days that a volume is considered as cleanup
	 *            candidate since it is detached
	 * @param retentionDays
	...
	public boolean isValid(Resource resource) {
		...
		Boolean valid = isValidResource(resource);
		...
		if (detachTime != null && detachTime.plusDays(detachDaysThreshold).isBefore(now)) {
			if (resource.getExpectedTerminationTime() == null) {
				...
				resource.setTerminationReason(
						String.format("Volume not attached for %d days", detachDaysThreshold + retentionDays));
				LOGGER.info(
						String.format("Volume %s is marked to be cleaned at %s as it is detached for more than %d days",
